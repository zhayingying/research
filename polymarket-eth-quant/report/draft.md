...
| 维度 | 核心判断 |
| --- | --- |
| 交易 | 交易的本质不是看对，而是当概率优势、赔率、成本、容量和风险预算同时成立时才下注。 |
| ETH | ETH 的本质是多层资金流状态机，短线看流动性和杠杆，中线看 ETF / 宏观 / 链上状态，长期看协议供需和监管定位。 |
| 数学 | 数学模型的本质是估计条件分布、置信度、尾部损失和可交易容量，不是做价格曲线拟合。 |
| 量化公司 | 顶级量化公司的本质是研究工厂加风控系统，数据、标签、回测、执行、组合和监控缺一不可。 |
...
| 交易门 | 本质问题 | 不通过时的处理 |
| --- | --- | --- |
| 生存门 | 这笔交易会不会让账户、策略或合规资格不可逆受损？ | 不交易。 |
| 期望门 | 概率优势是否覆盖价差、费用、滑点、冲击和机会成本？ | 降仓或放弃。 |
| 仓位门 | 这笔仓位是否符合波动率、流动性、相关性和风险预算？ | 重算规模。 |
| 复盘门 | 预测、执行、仓位和规则错误能否被分开记录？ | 不扩大规模。 |

可交易 edge 的判断式：

```text
p_model - p_market - cost
-> liquidity / capacity / risk budget filter
-> size / pass
```

也就是说，顶级交易员不是把“看对”直接变成下注，而是先把观点概率、市场价格、交易摩擦、成交容量和账户风险预算放到同一条链路里。链路任一处不通过，动作就应该是降仓或放弃。
...
| 驱动器 | 本质问题 | 典型观测 |
| --- | --- | --- |
| 边际资金流 | 有没有新钱进来或撤出？ | 宏观 risk-on/off、ETF flows、BTC beta、稳定币流动性 |
| 杠杆和流动性 | 同样一笔资金能推动价格多远？ | funding、OI、basis、清算、盘口深度 |
| 协议供需和叙事 | ETH 长期应被如何定价？ | staking、burn、gas、L2/blob demand、应用需求 |
| 事件重定价 | 原有概率分布是否突然失效？ | 升级、监管、安全事故、交易所风险、政策窗口 |

再按时间尺度拆开看：

| 时间尺度 | 主导问题 | 更该盯的变量 |
| --- | --- | --- |
| 分钟到日内 | 是否发生杠杆挤压或新闻跳变 | funding、OI、清算、BTC beta、盘口深度 |
| 数日到数周 | 边际资金流是否重定价 | ETF flows、basis、IV skew、监管/发行人新闻 |
| 月度 | 状态变量是否改变估值锚 | 宏观流动性、gas、burn、staking、链上 activity |
| 季度以上 | 协议经济和监管定位是否改变 | L2 demand、应用需求、竞争链、政策边界 |
...
先不要把数学领域当成平铺课程清单。落到交易系统时，可以分成四层：

| 层 | 目标 | 需要的知识域 |
| --- | --- | --- |
| 观测市场 | 对齐价格、盘口、资金费率、OI、IV、ETF flows 和事件流 | 时间序列、微观结构、数据工程 |
| 估计状态 | 估计趋势、波动、流动性、regime 和事件强度 | 统计推断、状态空间、随机过程 |
| 转成决策 | 把预测分布转成仓位、挂单/吃单选择、止损和组合约束 | 优化、Kelly、博弈论 |
| 验证和下线 | 判断模型是否仍能成本后交易 | 风险度量、因果推断、信息论 |

这四层真正汇合到一个中心对象：**条件收益分布 + action policy**。
输入层负责把市场状态看清楚，估计层负责从噪声中识别 regime，核心层输出未来收益的方向、幅度、波动和尾部，决策层才把它变成仓位、执行方式和退出条件。验证层负责证明它仍然可交易，否则下线。
...
| 拟合对象 | 含义 |
| --- | --- |
| 条件收益分布 | 在当前状态下，未来收益的方向、幅度、波动、尾部和置信度。 |
| 成本后期望 | 预测收益扣除 spread、fee、slippage、impact、fill probability 后是否仍为正。 |
| Action policy | 模型输出必须能转成买 / 卖 / 不动、挂单 / 吃单、仓位和撤单规则。 |
| 衰减监控 | 上线后默认 edge 会衰减，需要漂移监控、kill switch、重训和下线标准。 |
...
1. 固定数据合同：字段、时间戳、来源优先级、缺失值规则和复跑频率。
2. 定义标签和 horizon：分别定义 5m / 1h / 1d 的可交易收益标签，避免未来数据泄漏。
3. 建基线模型：先用 logistic / tree / simple ensemble 对比“市场隐含概率 + 成本”基线。
4. 设交易门禁：只有当 edge 覆盖 spread、fee、滑点、容量和风险预算时才允许下单。
...
这个样本的核心读法是：Yes 价格很低，但 ask 侧 top-5 深度明显小于 bid 侧。小额买入、较大规模建仓、以及之后退出，对应的成交条件不是同一件事；回测如果只按 midpoint 成交，会高估真实容量。
...
来源证据先分层，避免把官方规则、公共 API、论文方法和 vendor dashboard 混成同一等级：

| 证据层 | 用途 | 注意事项 |
| --- | --- | --- |
| 官方和监管来源 | 确认 Polymarket 规则、SEC order、Ethereum 协议机制和合规边界 | 后续使用前仍需重新核对 |
| 可复跑公共 API | Polymarket CLOB、Binance funding/OI、Deribit options summary | 可进回测，但要记录 venue bias |
| 论文和公开方法 | DeepLOB、101 Alphas、TSMOM、量化公司公开页面 | 只支撑方法框架，不声称内部流程 |
| Vendor / dashboard | ETF flows、liquidation history | 不从截图或反爬页面回填，需供应商或人工复核 |
